...
function handleSearch() {
  ...
  for (let i = 0; i < notes.length; i++) {
    ...
    const year = note.querySelector('.year').textContent.toLowerCase(); // Get the note subject

    // Check if the title or description includes the search term
    if (title.includes(searchTerm) || author.includes(searchTerm) || subject.includes(searchTerm) || year.includes(searchTerm)) {

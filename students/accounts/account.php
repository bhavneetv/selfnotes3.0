...
if ($res->num_rows > 0) {
    ...
    $progress_array = explode(',', $progress);
    $pro_count = count($progress_array);
    $class = $row['class'];
}
...
if ($result->num_rows > 89) {

    $subjects = [];
    while ($row = $result->fetch_assoc()) {
        $subjects[] = $row['subject'];
    }
    $new = array_merge($subjects, $subjects);
} else {
    $subjects = [];
    while ($row = $result->fetch_assoc()) {
        $subjects[] = $row['subject'];
    }
    $custom_subjects = ["Physics", "EVS", "Mathematics", "Biology", "Chemistry", "English", "History", "Science", "MP", "Hindi"];
    // $new = array_merge($subjects , $custom_subjects );
...
<body class="bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
    ...
    <div id="content" class="md:ml-72 transition-all duration-300">
        ...
        <div class="fixed inset-0 flex items-center justify-center p-4 animate-fadeIn   "
            style="display: none; z-index: 1000; margin: 14vh auto; " id="subjectPopup">
            <!-- Popup Card -->
            <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6" id="subjectPopupCard">
                <!-- Header -->
                <div class="mb-6">
                    <h2 class="text-xl font-semibold text-gray-800">Select subjects</h2>
                    <p class="text-sm text-gray-500 mt-1">Choose the subjects you're interested in</p>
                    <button id="closeBtn"><svg width="3 4px" height="34px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                            <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                            <g id="SVGRepo_iconCarrier">
                ...
                <form action="add_account.php" method="post">
                    ...
                    <div class="space-y-3 mb-6">
                        ...
                        <?php foreach ($uniqueSubjects as $subject): ?>
                            <label class="flex items-center p-3 rounded-md hover:bg-gray-50 hover:text-black transition-colors cursor-pointer">
                                <input type="checkbox" name="subject[]" value="<?php echo $subject; ?>"
                                    class="w-4 h-4 text-blue-500 border-gray-300 rounded focus:ring-blue-500" checked>
                                <span class="ml-3 text-gray-700"><?php echo $subject; ?></span>
                            </label>
                        <?php endforeach; ?>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-8 transition-colors duration-300">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="space-y-2">
                                <label class="block text-sm font-medium text-gray-600 dark:text-gray-400">Selected Subjects</label>
                                ...
                                <div class="flex flex-wrap gap-2">
                                    ...
                                    foreach ($myArray as $item) {
                                        echo "<span class='px-3 py-1 bg-indigo-100 dark:bg-indigo-900 text-indigo-600 dark:text-indigo-300 rounded-full text-sm'>$item</span>";
                                    }
                                    ?>
                                    <span class="px-3 py-1 bg-indigo-100 dark:bg-indigo-900 text-indigo-600 dark:text-indigo-300 rounded-full text-sm" style="cursor: pointer" onclick="openPopup(event)" data-subject="<?php echo $v; ?>">+</span>
                            ...
                            <div class="space-y-2">
                                <label class="block text-sm font-medium text-gray-600 dark:text-gray-400">Grade Level</label>
                                <select name="" id="classc" class="w-full px-4 py-2 rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-colors inp" disabled>
                                    <?php
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div class="bg-white dark:bg-gray-800 rounded-lg p-4 shadow transition-colors duration-300">
                                <div class="flex items-center justify-between">
                                    <div>
                                        <p class="text-gray-500 dark:text-gray-400">Completed Chapters</p>
                                        <p class="text-2xl font-bold text-gray-800 dark:text-white"><?php echo $pro_count; ?></p>
                                    </div>
                            ...
                            <div class="bg-white dark:bg-gray-800 rounded-lg p-4 shadow transition-colors duration-300">
                                <div class="flex items-center justify-between">
                                    <div>
                                        <p class="text-gray-500 dark:text-gray-400">Study Hours</p>
                                        <p class="text-2xl font-bold text-gray-800 dark:text-white"><?php echo $time; ?></p>
                                    </div>
    ...
    <script>
        ...
        function openPopup(event) {
            ...
            const d = new Date();
            d.setTime(d.getTime() + (86400 * 1));
            let expires = "expires=" + d.toUTCString();
            document.cookie = "subject=" + subject + ";" + expires + ";path=/";
            setTimeout(() => {
                window.location.href = '../../public/subjectUpload.php';
            }, 1000);
        }



        function infoUptade(n, e, g) {
